...
public class GameController implements ActionListener {
    ...
    private int numTimesSwapClicked;
    private int currentTurn;
    boolean swapTileSelected;
    private ArrayList<Integer> tilesToSwapIndices;
    private int numTilesPlacedThisTurn;
    private int tileIndex;
    boolean aTileSelected;
    ...
    public GameController(GameModel model, GameView view) {
        ...
        tilesInBag -= 7 * model.getPlayers().size();
            ...
            for (int row = 0; row < 15; row++) {
                for (int col = 0; col < 15; col++) {
                    if (view.getSpecificBoardCellColour(row, col) == Color.GRAY) {
                        view.setSpecificBoardCellColour(row, col, Color.GREEN);
                    }
                }
            }
